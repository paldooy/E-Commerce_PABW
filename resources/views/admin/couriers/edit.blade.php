...
<div class="glass-card mx-auto max-w-xl rounded-2xl p-6 shadow-sm">
    <h1 class="mb-4 text-2xl font-semibold">Edit Kurir</h1>
    <form method="POST" action="{{ route('admin.couriers.update', $courier) }}" class="space-y-4">
        ...
        <div>
            ...
            <input name="nama_lengkap" class="mt-1 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-slate-100" value="{{ $courier->nama_lengkap }}" required />
        </div>
        <div>
            <label class="text-sm font-medium text-slate-300">Email</label>
            <input type="email" name="email" class="mt-1 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-slate-100" value="{{ $courier->email }}" required />
        </div>
        <div>
            <label class="text-sm font-medium text-slate-300">Nomor HP</label>
            <input name="no_hp" class="mt-1 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-slate-100" value="{{ $courier->no_hp }}" required />
        </div>
        <div>
        ...
        <div class="flex items-center gap-2">
            ...
            <input type="checkbox" name="status_aktif" value="1" {{ $courier->status_aktif ? 'checked' : '' }} />

            ...
            <div class="glass-card rounded-2xl p-4 shadow-sm">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        @if($order->items->first() && $order->items->first()->gambar_produk_snapshot)
                            <img src="{{ $order->items->first()->gambar_produk_snapshot }}" alt="Produk" class="h-16 w-16 rounded-lg object-cover" />
                        @else
                        ...
                        <div>
                            ...
                            <p class="text-base font-semibold">{{ $order->items->first() ? $order->items->first()->nama_produk_snapshot : 'Pesanan' }} {{ $order->items->count() > 1 ? '(+ '.($order->items->count() - 1).' produk)' : '' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="glass-card rounded-2xl p-4 shadow-sm border border-slate-800/50">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-xs capitalize {{ $order->escrow_status === 'dikembalikan' ? 'text-rose-400' : 'text-emerald-400' }}">{{ str_replace('_', ' ', $order->escrow_status) }}</p>
                            </div>
                        </div>
                    ...
                    <div class="glass-card rounded-2xl p-4 shadow-sm border border-slate-800/50">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs capitalize {{ $item->status === 'transaksi_gagal' ? 'text-rose-400' : 'text-emerald-400' }}">{{ str_replace('_', ' ', $item->status) }}</p>
                            </div>
                        </div>
                    </div>
                @empty
